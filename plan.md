...
唯一运行主链：联合监测区请求 → 原候选联合搜索快速选成员、方法、路线及支援时序 → 一份 Plan → 原 runner 并行派发 Action → Swarm AIR、qn 跨介质/qn WATER UUV、Otter/PVS USV → 实际 Result 与母船收到的结果 → 至多一轮缺测修复。固定五平台，允许冗余 AAV 待命，不强制编队。通信只按声明的 AIR 直接接触与 UUV–USV–母船接力几何评价，不研究字节和协议。
...
场景的业务活动是：一台合格 AAV 穿过联合区中的 6×4 m 空中扫测带四位置，另一台 AAV 做 AIR→WATER→AIR 浅水点测，UUV 做同区三位置深水通过段，USV 在选中的接触位置接收 UUV 结果并向母船交付，参与成员返回。四份空中、浅水一份、深水三份均须实收。几何足迹是当前观测代理，不代表真实载荷影像质量。USV 使用 Otter 原生 LOS 航点控制，返回只在实际进入部署区后完成。

## 当前证据

- `experiments/20260925-joint-area-live-r20`：默认预算下初次规划约 1.15 秒，同次实时 RViz/中文面板四活动并行，四空中＋四水下产品到母船，跨介质、近岸接触支援和规定返回完成，`PASS_GEOMETRIC_PROXY_QUALIFICATION`；同次五平台 bag 审计 `passed=true,failures=[]`，最小代理净距约 1.372 m；`contact-audit.json` 核八产品接收位置。
...
- `experiments/20260925-joint-area-retest-r21`：最终八结果模板的一次明确缺测复查正在实跑，完成前不记通过。旧 `area-sweep-retest-r16` 因新增分组代码局部变量未赋值在修复入口失败，未作复查成功证据；该错误已在同处修正。

## 下一步

1. 完成 r21，核对负报告先于补测、新成员和必要支援实际派发、全部八结果收件、所有返回及同次独立 bag 审计。若失败只修该执行边界。
