...
if [[ "$VISUALIZE" == true ]]; then DEFAULT_SCENE=five_scene_harbor.yaml; fi
...
docker run "${GUI_ARGS[@]}" --rm --init --env ROS_HOME=/tmp/five-ros --env FIVE_VISUALIZE="$VISUALIZE" \
  ...
  "$FIVE_PLATFORM_IMAGE" bash -c '
    ...
    roslaunch qn_aav_simulator five_qualification.launch scene_file:=/experiments/scene.yaml visualize:="$FIVE_VISUALIZE" > /experiments/current/launch.log 2>&1 &
    launch_pid=$!
    ...
    if [[ "$FIVE_VISUALIZE" == true ]]; then
      ...
      python3 /workspace/src/src/qn_aav_simulator/scripts/mission_dashboard.py _planning_mode:=five_qualification _window:=true _rate:=2 > /experiments/current/dashboard.log 2>&1 &
