## 2026-09-25 — 在线候选评价与完整动力学回放职责复核

- 计划：回应用户“评估函数应快速选择合适候选、不能逐候选重跑执行仿真”，只核对代码与直接文献，不继续扩大长预算试跑；持续目标工具当前显示paused，未恢复目标。
- 实际：只读核查原请求/候选/worker及Calvo T-RO 2025、D-ITAGS RA-L/IROS 2023、GRSTAPS IJRR 2022原文。确认当前AIR候选先Swarm查询再qn完整积分，跨介质和UUV/USV也推进完整动作，完整叶还积分待命尾段；真实AIR执行时又由Swarm重新规划，PVS端另有完整本地preflight。`evaluated_candidates=3`计的是区域方法候选，不是3份完整五平台方案，纠正此前进度表述。完整闭环回放不应成为在线分配评价的统一前置；联合性应保留在成员/方法/资源/协作窗口/反馈上。
- 运行处置：中断前遗留r21后台Docker仍运行。首次读metrics为PLANNING_DIAGNOSTIC；停止前已在214.077s形成Plan并通过先前管道确认进入四活动RUNNING，因此仅登记为主动中止，不能称零Goal或通过。Docker已停止，包装脚本退出，systemd-timesyncd恢复active。未新增仿真实验，也未改生产算法。
- 来源：Calvo https://arxiv.org/html/2411.02062v3 （§IV-A估计转场、§V启发式、§VI执行与修复）；D-ITAGS https://arxiv.org/pdf/2209.13092 （§V-B估计/按需PRM/路径复用、§V-C针对性修复）；GRSTAPS https://journals.sagepub.com/doi/10.1177/02783649211052066 （§4交错规划、§5无动力学Lazy PRM实验、§6.1碰撞边界）。这些不是本工程全物理安全保证。
- 下一步：在原候选与Plan里用当前状态/合格方法、地图可达路线、资源及支援关系做快速筛选和低成本评分；选中动作交原端点实际规划控制，未接纳、延迟/失败、缺测等反馈触发未承诺部分修复。全动力学回放退为离线资格/必要边界验证；同步修改依赖terminal_backend、密集轨迹和假定预测终态的派发/修复接口，不能只删检查或填假FEASIBLE。此架构收敛尚未实施，现有未提交工作树保留。

## 2026-09-24 UTC — 同请求缺测复查诊断进行中

- 计划：从已通过的三类正常请求出发，在`drone_0`首个AIR观测Action只注入一次明确缺测，要求本机负报告实际抵母船后，同一runner用第三台AAV与已返回USV重排、派发新Goal、实际收件和返回；初次/修复正式预算仍默认10s，长预算仅作断点诊断。
- 实际：`formation_action_server.py`的显式资格参数`/mission/qualification_missing_air_member`只跳过该次本机观测采样，不改运动、障碍、结果或runner事实。`r12`首轮AIR缺测已实收，但跨介质AAV AIR返程虽运动/安全PASS，0.35s状态采样缺口使实验有效性INVALID并锁定，未派复查；保持原门槛。`r13`使用此前正常正例核组后首轮四活动有效完成，AIR结果为`OBSERVATION_MISSING`；修复前的USV完整状态摘要与名义终态不同，runner正确拒绝，零新Goal。
- 修正：现有PVS本机`state_digest`服务增加Otter所需的**数值**状态快照（eta、nu、执行器、自动驾驶可变记忆、模型计数），母船状态收件按已有事件转发；原`PvsBackend`私有查询副本按白名单重建并核对完整摘要，不把旧名义模型伪装成实际状态、不在Goal复制模型。独立往返原生Otter动作后的状态重建/摘要一致检查PASS。已有qn本机摘要服务也用于固定WATER UUV状态领取；其余已返回成员以有界、在线监测的占用球继续进入完整计划安全校核。现有AIR候选补入任务级USV再支援方法，仍须实际验证。
- 证据：`experiments/20260924-qn-uuv-air-retest-r{12,13}`的metrics/Result/诊断；改动限原`formation_action_server.py`、`pvs_node.py`、`pvs_backend.py`、`scene_publisher.py`、`executors.py`与runner；新增一项实际状态摘要往返检查在现有`test_native_motion_query.py`。`r14`已开始复跑，尚无复查成功证据。
- 未完成／下一步：读取r14首轮、状态领取与补测Plan/Goal结果；正式10s初次/修复和同次复查仍未通过。不能把单个负报告或新接口存在写成闭环成功。
...
- 实际：在原`PvsNode`内把预检使用的本机plant/控制器/执行器完整对象（仅剔除不会改变未来动力学的本地`time_s/steps`计数）按同一pickle协议取签名；在`_preflight`结束、取得ActionServer和模型锁后与实时本机状态核对，不一致则拒绝旧查询。已PREPARED的Goal在`start_prepared`再次核对同一签名；有场景却没有原生预检证据的片段不能启动。原中文面板仅对这三种新失败原因作短中文映射。未新增服务、消息、协调节点或业务阈值；后端真实积分和原场景安全判据不变。
- 针对性负/正证据：`experiments/20260924-pvs-preflight-state/race_probe.py`用真实Otter模型先推进一个位置**仍完全相同**但内部状态变化的步，受控查询返回FEASIBLE仍被`NATIVE_STATE_CHANGED_DURING_QUERY`拒绝；同脚本对已预装Goal的状态变化拒绝`NATIVE_STATE_CHANGED_SINCE_PREPARATION`，缺资格签名拒绝`NATIVE_PREPARATION_NOT_QUALIFIED`。真实Noetic/Otter静态配平约9模型秒后，单Goal获SUCCEEDED、`terminal_verified=true`、锁空；`experiments/20260924-pvs-preflight-prepared`同样从PREPARED经标准`StartPreparedAction`获`START_ACCEPTED`和成功Result。两次原生Action均录Goal/Result/诊断bag。
- 三类同请求实时回归：`experiments/20260924-three-class-state-guard-live-r1`用原港口/回区、仅资格测试配置暂不提供AAV水下备选、180s显式诊断规划，终端自动输入`yes`后真实RViz/中文面板同ROS会话执行AAV1 AIR、REMUS原生水下样点与移动Otter预承诺支援。两海洋Goal均PREPARED后共同启动，新状态守卫未误拒；任务权威`PASS_GEOMETRIC_PROXY_QUALIFICATION`、空中/水下两业务均有母船收件、4/4原生Goal/Result、6/6有限命令、4/4有限终态母船通知、三参与者规定返回、零锁。面板保存规划、执行、100%收件仍占USV、终态，RViz对应真窗口图。
- 独立全场证据：同次`scene-once.bag`＋`execution.bag`运行`audit_swarm_roundtrip.py --include-marine`得`passed=true,failures=[]`；3115对齐位置采样零缺、模型/ROS峰值0.021332s<原0.05s、五平台主管代理最小净距0.593172m>0.5m、UUV实体最小余量0.681069m>0.2m。独立同bag命令/Goal/Result/产品因果审计`passed=true`，四Goal四Result四母船终态、六请求六送达、两必要摘要。`systemd-timesyncd`实验前后均active，宿主服务临停仅为已核时钟回拨负例的有界实验条件。
- 范围与下一步：本次证明**已声明初态、已资格静止入口及几何代理任务**在新守卫下可同次实时完成；普通完整能力请求仍可能选AAV而UUV待命，180s不是原10s正式规划，未触发缺测复查或证明任意运动中方法的重新资格。新签名守卫对运动中状态改变会明确拒绝陈旧预测；下一步必须在既有执行端从实际未来采用状态作查询并保持母船信息边界，不能靠位置接近或任意容差跳过。最后新增的“缺资格签名拒绝”支路在受控负例通过，发生于该次长实时回归后，不冒称已被长运行覆盖。
- 代码/命令/证据：`integration/qn_aav_simulator/scripts/pvs_node.py`；`experiments/20260924-pvs-preflight-state/{race-result.json,action-result.json,execution.bag}`、`experiments/20260924-pvs-preflight-prepared/{prepared-result.json,execution.bag}`、`experiments/20260924-three-class-state-guard-live-r1/{metrics.json,nominal-plan.json,scene-once.bag,execution.bag,safety-audit.json,control-audit.json,dashboard-running.png,dashboard-delivered.png,dashboard-terminal.png,rviz-running.png,rviz-terminal.png,clock-service-before.txt,clock-service-after.txt}`；命令为`bash experiments/20260924-pvs-preflight-state/run.sh`、`bash experiments/20260924-pvs-preflight-prepared/run.sh`及`printf 'yes\n' | bash scripts/docker_run_three_class_qualification.sh <输出目录>`，审计命令/输入保留在同目录`audit.log`、`control_audit.py`。
...
- 实际：直接在现有`QnPythonClosedLoopBackend.predict_idle()`的**只读查询**内，仅对`INITIAL_HOLD`用两次真实0.01s外层积分，逐字节比较连续步的整个plant/控制器/执行器/参考记忆、并要求全部内层样本位置与固定点相同；若精确相等且静态场景初态有效，则根据确定性常输入转移`F(x)=x`归纳生成同一时网格的待命轨迹和全状态终点；任何差异、预算到期或非初始保持仍落回原逐步积分/UNKNOWN。在线`qn_aav_node.py`**继续每一步真实积分**，不虚增实际模型时间。原完整计划验证中，对轨迹严格逐样本相同的固定成员只把相同场景与固定—固定净距谓词算一次，时间缺样检查与所有涉及运动成员的场景/机间关系照旧全程检查；没有新增缓存管理器或接口。
- 正确性核对：AAV3原港口静态配平连续两步的完整backend状态完全相同，20模型秒原逐步查询后的唯一状态变化只是首步初始化的诊断参考/加速度；新3模型秒只读预测与300次显式积分的完整backend状态和301项位置时网格一致。针对性任务/PVS/qn/完整候选26项通过。300模型秒静态待命查询从此前按秒级逐步计算变为约0.0067s，但这仅在同一源码精确固定点成立；不是通用飞行稳定性或抗扰动证明。
- 10秒名义小例：本机固定五模型、原港口水下组件请求（仅测试资格标签使UUV/USV方法必被查，三个AAV真实待命参与整份安全检查）在**现有**`build_request_executor_plan`、同一PVS/FiniteDelivery/固定场景下，旧源码10s返回`PlanningBudgetExceeded`、20s约12.23s得到计划；新固定点与静态谓词复用在10s约10.012s墙钟返回一份完整FEASIBLE计划、2候选，`search_complete=false`，20s约12.92s评6候选。额外少量线程并行试验反而使两PVS查询从6.41s变7.05s，未加入生产。10.012s是带收尾开销的一次本机结果，**不能**把它宣称为严格10s正式ROS通过或最优性证书。
- 来源/证据：上述固定点不是外部论文阈值，而是现有`qn_python_backend.py::step/reset/predict_idle`确定性方程与全内部状态相等的可复核归纳；真实源码和显式逐步对照见`test_native_motion_query.py::test_qn_initial_trim_fixed_point_matches_every_explicit_model_step`。本机忽略目录`experiments/20260923-remus-original-return-search/{fast-qn-standby-plan-cost.json,fast-qn-plan-cost-r2.json,first-candidate-cost.json,native-parallel-cost.json}`；现有`executors.py::_check_complete_plan`谓词只在相同输入时复用。文献[GRSTAPS](https://journals.sagepub.com/doi/full/10.1177/02783649211052066)启发运动反馈要进完整计划校核，但没有给本机固定点结论或10s性能保证。
- 未完成／下一步：重建匹配的新Noetic镜像，运行默认10s正式ROS入口并保留成败；随后对带AAV AIR+UUV/USV的两区域全请求实测10s。若后者仍超预算，不得借本小例降低任务/安全要求宣布达标；初次离线预算政策与UUV独有业务资格的用户选择仍待答，真实缺测复查/在线全状态修复也未完成。
...
## 2026-09-25 — 快速任务级联合分工、同次三类实跑与区域扫测

- 计划：让在线求解只选成员、方法、已声明路线和共享支援时序，原 Swarm/qn/Otter Action 负责实际运动；在同一实时 RViz／中文面板完成正常请求及一轮明确缺测复查。随后按用户反馈把“到点即返”的空中点测改为有限区域扫测。
- 实际：`OFFSHORE_JOINT` 在原 `executors.py` 读取实际 Odometry、模式和占用，以静态可达路线及名义时序快速筛选整份方案，默认初次与修复共用各自 10 秒预算；预测标记 `TASK_LEVEL_EXECUTION_PENDING`，不再逐候选重跑 Swarm/qn/PVS 动力学。原 runner 并行派发、保留同成员多步预约、按 USV 实际到位交付并等待真实报告；原端点核当前入口。r1–r6 暴露水面 z 入参和短暂远端状态缺口；r6 的 AAV/USV 实际净距越过 0.5 m，改用场景已声明的南侧 USV 航路，不降安全门槛。r9 的备用 AAV 贴防波堤失败后，空中方法改走场景安全通道，并作为真实连续 Action 派发；r11 暴露中间航点未稳定，复用原 4 秒保持。r12 终结后 CSV 落盘延迟被误算成执行采样尾段，在原 Action 记录监测器实际终结时刻，不改模型时间和采样阈值。
- 结果：`experiments/20260925-task-level-live-r8` 初次规划 0.452 秒，AAV AIR、另一 AAV AIR→WATER→AIR、qn UUV 与 Otter USV 从同一 Plan 并行执行，三份产品实收、四活动与规定返回完成、零锁、任务 `PASS_GEOMETRIC_PROXY_QUALIFICATION`；独立同次 bag 审计 `passed=true,failures=[]`，五平台代理最小净距 1.491655 m、模型/ROS 最大漂移 0.018228 秒。`experiments/20260925-task-level-retest-r13` 初次规划 0.946 秒，明确空中缺测报告到达后修复规划 0.232 秒，选待命 AAV2＋已返回 USV 实际补测、收件和再次返航，`retest_completed` 一项、任务 PASS；同次独立审计 `passed=true,failures=[]`，最小净距 1.401911 m、最大模型/ROS 漂移 0.015435 秒。两次搜索均未穷尽，不能称最优；观测和通信仍分别是几何代理与任务级支援假设。
