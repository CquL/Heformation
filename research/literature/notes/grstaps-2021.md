...
> **阅读范围声明：仅摘要。** IJRR 正式版在 SAGE 付费墙后；Unpaywall（DOI 10.1177/02783649211052066）返回 `is_oa=false`、无 OA 位置，OpenAlex 返回 `oa_status=closed`，作者 GitHub（GT-STAR-Lab/GRSTAPS）只含实验代码，无论文 PDF。因此本笔记只依据 STAR Lab 落地页摘要与仓库描述，**不引用正文定理、算法细节或实验数字**。
