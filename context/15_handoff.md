...
> **2026-09-23 依据审计继续点**：先读 `docs/reviews/source-and-assumption-audit-20260923.md`。删除了无业务依据的默认回起点/以入口容差代替回收区域，场景或调用现在必须给出回收位置及半径；当前联合请求缺这一输入，不能默认接受。AIR服务时间已由请求驱动，固定多一秒USV支援和重复AIR尾段Action已撤。旧水下0/30/40秒等待为实验候选，不是文献规律。仍需把全AIR/WATER请求在10秒预算内形成可行计划、执行并经中文实时显示，后续一次缺测复查/返回尚未验收。
