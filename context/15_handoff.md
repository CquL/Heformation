...
- 但**累计时间门槛不过**：`model/ROS rate` 0.99928，72.8 s 累计偏差 57 ms > 50 ms。这是感知链真正跑起来以后（7 渲染器 + 7 规划器 + 7 qn 节点挤在 CPU 受限的宿主上）的负载结果；单任务对齐报告通过，累计的不通过。
- 按冻结基线的停止规则，**不进入盒子避障归因**，也未经调整盒子或避障权重。第 3、4 步（原盒子时间线、局部扫描中断）尚未执行；局部扫描中断所需的"运行中扫描停更即判故障"规则已在代码中实现并通过单测。
