...
> **2026-09-25 当前交接（优先于下文历史）**：主链已改为现有 Heformation 的快速任务级联合候选，默认 10 秒内选 AAV 空中扫测／AAV 浅水点测／UUV 深水通过／USV 支援与各自时序，原 runner 并行派给 Swarm/qn/Otter，母船只按实际收件结项。最终八结果联合区正常请求 `experiments/20260925-joint-area-live-r20`同次实时 RViz/中文面板 PASS，规划约 1.15 秒，八产品实收、四活动及返回完成、五平台独立 bag 安全/时间审计 PASS；独立接触距离核对在 `contact-audit.json`。近岸 USV 点由有限候选按实际 USV 位置、UUV 返回处与母船位置排序，结果仅在实际 UUV–USV/USV–母船接触（声明 8/30 m）后到达；AAV 在 AIR 且接近母船后直接交付。旧远航固定点与逐字节传输不在本轮运行主链。当前运行 `experiments/20260925-joint-area-retest-r21`检验一次负报告后的同请求修复，完成前不宣称最终闭环。UUV 仍为 qn WATER 代理；几何观测/任务级接触是实验假设，不能宣称真实载荷、声学/RF设备性能或严格物理安全保证。源码工作树未提交；继续先读 WORKLOG 顶部和 plan.md，阶段结束再清旧 v9 专属代码、更新 README 并推 main。下文旧状态仅保留历史，不覆盖本段。

> **2026-09-24 最新交接（优先于以下历史）**：新远域模板已在`experiments/20260924-qn-uuv-joint-live-r11`同次实时ROS/RViz/中文面板通过正常三类协同：一台AAV AIR、一台AAV跨介质、qn固定WATER的UUV代理、Otter USV共享支援，四活动并行启动且实际完成，三项结果母船收到、四成员回区、零锁；同次五平台bag/场景独立审计PASS。UUV由用户改定为qn水下代理，不再称REMUS验证。现有任务权威与执行链仍是一套；无额外协调节点。**剩余关键**：同一三类请求下收到明确缺测后的一轮复查尚未实跑，初次完整规划默认10s仍无方案（成功运行用了300s隔离诊断且搜索未穷尽），旧Calvo专用分支与文档待按调用关系清理。现用入口/限制见README和WORKLOG首条；工作树未提交/推送。以下旧“无Plan／等待PVS答复／REMUS执行”均是过时历史口径。

> **2026-09-24 协同模板实施交接**：当前未提交工作树已接三观测角色、共享USV支援、三AAV同型跨介质端点、任务级结果收件及动态AIR邻机只读查询。新远域场景完整请求仍无可提交Plan：10秒预算0候选，长预算至少见AAV动态路径冲突和UUV原生长回环不能回部署区。PVS引导修正范围的异步用户决定未到前不要改变其控制；也不要把局部方法、角色前缀或RViz初始画面写为联合任务成功。下一步从WORKLOG首条与当前源码状态继续，先解决REMUS返区与两AAV联合资格，再做一次同会话实时闭环和反馈修复；旧Calvo专属代码须在新主链验收后按调用关系清理。
