...
> **2026-09-23 AAV备选继续点**：`experiments/20260923-aav-water-method/result.json`已有同一qn全状态的AIR转场74.55秒、垂直入水/短水下观测/出水54.91秒、AIR回部署区72.36秒三段只读FEASIBLE；水中实际模型样本产生`water_sample`几何代理。它还未进入现有候选联合比较、AIR/平台复合Action、有限交付或同次实际任务，不能替代UUV返航失败。下一步直接在原请求/候选/runner上接有限跨介质方法，不加新调度框架，并保留10秒预算限制的UNKNOWN口径。
